      ...
      <div class="input-box">
        ...
        <form @submit.prevent="submit" style="margin-top: 0.75rem">
          ...
          <div class="loading">
            ...
            <p class="status_text" style="color: rgb(22 163 74)" v-if="success" >{{ success }}</p>
            <p class="status_text" style="color: rgb(220 38 38)" v-else-if="error">{{ error }}</p>
          </div>
        </form>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    //   console.log(this.$route.query.token);
      ...
      this.success = "";
      this.axios
        .post(`${BASE_URL}${this.endpoint_path}?token=${this.$route.query.token}`, {
        ...
        .then(
          (response) => {
            ...
            try {
                ...
                console.error(`Failed to parse JSON response from server: ${JSON.stringify(response.data)}`);
                console.error(e);
            }
          },
          (response) => {
